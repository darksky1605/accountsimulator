...
namespace ogamehelpers{
	...
	int getItemProductionPercent(ItemRarity item){
		switch(item){
			case ItemRarity::Bronze: return 10;
			case ItemRarity::Silver: return 20;
			case ItemRarity::Gold: return 30;
			case ItemRarity::None: return 0;
		}
		return 0;
	}
	...
	Production getDailyProduction(int metLevel, ItemRarity metItem, int metPercent, 
		...
		if(result.met < 0) result.met = std::numeric_limits<int>::max();
		if(result.crystal < 0) result.crystal = std::numeric_limits<int>::max();
		//if(result.deut < 0) result.deut = std::numeric_limits<std::int64_t>::max();
